...
turtles-own [
  ...
  Tenacity            ;;改变自身意见的难易程度
  next-Opinion        ;;下个时间步的opinion值
]
...
to setup
  ...
  set global-opinion-last opinion-last
  set global-goal goal
  reset-ticks
end
...
to model-run
  if ticks > tick-number and tick-number != 0
  [ stop
    continue
     ]
  ask turtles [                                     ;;交互过程，所有agent同时参与
    let opinion-i Opinion
    let num-i who
    let neighb-set link-neighbors with [ abs(Opinion - opinion-i) < limit]
    ifelse any? neighb-set
    [ let tmp 0
      ask neighb-set [
        let num-j who
        set tmp tmp + (Opinion - opinion-i) * [Trust] of link num-i num-j * Tenacity * Influence
      ]
      set next-Opinion opinion-i + (tmp / (count neighb-set))
    ]
    [ set next-Opinion opinion-i]
  ]
  ask turtles [
...
to init-agents
  let d-max max [count link-neighbors] of turtles
  ask turtles [
    set Opinion random-float 2                      ;;设置opinion值
    set Influence (count link-neighbors) / d-max    ;;设置Influence值
    set Tenacity random-float 1
    change-states
  ]
  ask links [                                       ;;设置Trust值
    set Trust random-float 0.2
  ]
  let num count links
  ask n-of (num * 0.2) links [
    set Trust (random-float 0.8) + 0.2
  set color gray + 2
  ]
end

to continue
  if global-opinion-last != opinion-last
  [ set global-opinion-last opinion-last ]
  if global-goal != goal
  [ set global-goal goal ]
  let leader-num number * opinion-leader / 100
  ask max-n-of leader-num turtles [Influence] [
    set Opinion global-opinion-last
    change-states
  ]
  tick
end
...
"" ""
PENS
"反对" 1.0 0 -13345367 true "" "plot count turtles with [color = blue]"
"中立" 1.0 0 -2674135 true "" "plot count turtles with [color = red]"
"支持" 1.0 0 -10899396 true "" "plot count turtles with [color = green]"
...
385
o-percent %
count turtles with [color = blue] / number * 100
17
1
15
...
462
m-percent %
count turtles with [color = red] / number * 100
17
1
15
...
536
s-percent %
count turtles with [color = green] / number * 100
17
1
15
